...
export class MongoRepository<T extends Document> implements IRepository<T> {
  ...
  async find<TFil = RootFilterQuery<T>, TOp = QueryOptions>(
    filter: TFil,
    options?: TOp,
  ): Promise<T[]> {
    return (
    ...
    ).map((u) => u.toObject({ virtuals: true }));
  }
  ...
  @ConvertMongoFilterToBaseRepository()
  async findOne<TFil = RootFilterQuery<T>, TQue = QueryOptions>(
    filter: TFil,
    options?: TQue,
  ): Promise<T | null> {
  ...
  @ConvertMongoFilterToBaseRepository()
  async findAll<TFil = FilterQuery<T>, TOpt = QueryOptions<IEntity>>(
    filter?: TFil,
    options?: TOpt,
  ): Promise<T[]> {
    const modelList = await this.model.find(
      filter as FilterQuery<T>,
      options || ({} as QueryOptions<IEntity>),
    );
  ...
  @ConvertMongoFilterToBaseRepository()
  async remove<TQuery = FilterQuery<T>, TOpt = unknown>(
    filter: TQuery,
    options?: TOpt,
  ...
  async findOr<TOptions = QueryOptions>(
  ...
  >(
    ...
    if (filter) {
      (filter as FilterQuery<T>) =
        this.applyFilterWhenFilterParameterIsNotFirstOption(filter);
    }
  ...
  >(
    ...
    if (filter) {
      (filter as FilterQuery<T>) =
        this.applyFilterWhenFilterParameterIsNotFirstOption(
          filter as FilterQuery<T>,
        );
    }
