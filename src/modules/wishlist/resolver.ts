...
export class WishlistResolver {
  ...
  async productExists(@Args('name', { type: () => String }) productName: string, @Context() ctx: { req: { user: UserEntity } }) {
    return this.existsWishlistUsecase.execute({ productName, user: ctx.req.user })
  }
