...
export class WishlistController {
  ...
  async list(@Req() { body, user }: ApiRequest): Promise<WishlistListOutput> {
    ...
    if (wishlistExists) {
      return this.listWishlistUsecase.execute({ ...body, user: user } as WishlistListInput)
    }
    throw new ApiNotFoundException(`wishlist from user: ${user.name.toUpperCase()} not found.`)
  }
  ...
  async exists(@Req() { params, user }: ApiRequest): Promise<WishlistProductExistsOutput> {
    const wishlistExists = await this.repository.findOne({ user: { name: user.name.toUpperCase() } })
    if (wishlistExists) {
      return this.existsWishlistUsecase.execute({ ...params, user: user } as WishlistProductExistsInput)
    }
    throw new ApiNotFoundException(`wishlist from user: ${user.name.toUpperCase()} not found.`)
  }
  ...
  async delete(@Req() { params, user }: ApiRequest): Promise<WishlistRemoveOutput> {
    const wishlistExists = await this.repository.findOne({ user: { name: user.name.toUpperCase() } })
    if (wishlistExists) {
      return this.removeWishlistUsecase.execute({ ...params, user: user } as WishlistRemoveInput)
    }
    throw new ApiNotFoundException(`wishlist from user: ${user.name.toUpperCase()} not found.`)
  }
}
